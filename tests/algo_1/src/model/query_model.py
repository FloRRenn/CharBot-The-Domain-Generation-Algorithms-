...
class QueryModel:
    ...
    def predict(self, domain_name) -> bool:
        """
        ...
        """
        predict_df = pd.DataFrame(domain_name, columns=self.input_columns)
        results = self.model.predict(predict_df)
        return {"isBenignDomain" : all(predict_df["result"] == self.desired_class), "type" : predict_df['domain'].to_dict()}
